...
pub mod pallet {
    ...
    pub type LockedAmounts<T: Config> = StorageDoubleMap<
        _,
        Blake2_128Concat,
        T::AssetId,
        Blake2_128Concat,
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::call_index(6)]
        #[pallet::weight((<T as Config>::WeightInfo::lock(), Pays::No))]
        pub fn lock(
            origin: OriginFor<T>,
            #[pallet::compact] id: T::AssetId,
            beneficiary: T::AccountId,
            ...
            let origin = ensure_signed(origin)?;
            Self::lock_tokens(id, beneficiary, amount)?;
            Ok(())
        }

        #[pallet::call_index(7)]
        #[pallet::weight((<T as Config>::WeightInfo::unlock(), Pays::No))]
        pub fn unlock(
            origin: OriginFor<T>,
            #[pallet::compact] id: T::AssetId,
        ) -> DispatchResult {
            let origin = ensure_signed(origin)?;
            Self::unlock_tokens(id, origin)?;
            Ok(())
        }

        #[pallet::call_index(8)]
        #[pallet::weight((<T as Config>::WeightInfo::burn_tokens(), Pays::No))]
        pub fn burn_tokens(
            origin: OriginFor<T>,
            #[pallet::compact] id: T::AssetId,
            beneficiary: T::AccountId,
            ...
            let origin = ensure_signed(origin)?;
            Self::burn_token(id, &beneficiary,Some(origin))?;
            Ok(())
        }
