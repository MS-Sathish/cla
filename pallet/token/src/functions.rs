...
impl<T: Config> Pallet<T> {
    ...
    pub(super) fn burn_token(
        ...
        let locked_amount = LockedAmounts::<T>::get(id, target);
        ensure!(locked_amount > Zero::zero(), Error::<T>::InsufficientTransfer);
    
        // if let Some(check_admin) = maybe_check_admin {
        //     let details = Asset::<T>::get(id).ok_or(Error::<T>::AssetNotFound)?;
        //     ensure!(check_admin == details.issuer, Error::<T>::NoPermission);
        // }
    
        LockedAmounts::<T>::mutate(id, target, |locked| {
            *locked = Zero::zero(); 
        });
    ...
    pub(super) fn lock_tokens(
        id: T::AssetId,
        owner: T::AccountId,
        amount: T::Balance,
    ) -> DispatchResult {
        ensure!(
            <BalanceOf<T>>::contains_key(id, &owner),
            Error::<T>::AccountDoesNotOwnThisToken
        );

        <BalanceOf<T>>::try_mutate(id, &owner, |balance| -> DispatchResult {
            ensure!(*balance >= amount, Error::<T>::InsufficientTransfer);

            *balance = balance.checked_sub(&amount).ok_or(Error::<T>::TransferUnderFlow)?;

            <LockedAmounts<T>>::try_mutate(id, &owner, |locked| -> DispatchResult {
                *locked = locked.checked_add(&amount).ok_or(Error::<T>::BalanceOverflow)?;
                Ok(())
            })?;
    ...
    pub(super) fn unlock_tokens(id: T::AssetId, owner: T::AccountId) -> DispatchResult {
        ensure!(
            <LockedAmounts<T>>::contains_key(id, &owner),
            Error::<T>::NoPermission
        );
    
        <LockedAmounts<T>>::try_mutate(id, &owner, |locked| -> DispatchResult {
            ensure!(*locked > Zero::zero(), Error::<T>::NoLockedAmount);
